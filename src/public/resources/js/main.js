...
/*Fetch all tasks from API and add them to DOM lists*/
getTasks((tasks) => {
    const combinedTasks = tasks.todo.concat(tasks.completed);

    if (!Array.isArray(combinedTasks)) {
        console.error("Combined tasks is not an array:", combinedTasks);
        return;
    }

    combinedTasks.forEach((item) => {
        console.log(item);
        addItemPage(item, item.completed);
    });
});
...
function completeItem() {
    ...
    //If statement of whether it should be added to completed or todo list
